...
Typical live order:
  ...
  2. Edit gitops/overlays/tta/ingress-host-patch.yaml only when the instructor assigns an ingress host
...
check_manifests() {
  ...
  if command -v kustomize >/dev/null 2>&1; then
    ...
    if grep -q "REPLACE_WITH_YOUR_INGRESS_DOMAIN" /tmp/ai-quality-kserve-rendered.yaml; then
      echo "[warn] Ingress host still has the course placeholder. Keep it for manifest inspection unless the instructor assigns an ingress host; classroom UI access uses the Bastion tunnel."
    fi
  else
...
sync_app() {
  if ! command -v argocd >/dev/null 2>&1; then
    ...
    echo "Fallback: inspect demos/ch03_docker_kubernetes/argocd/application.yaml and gitops/overlays/tta." >&2
    exit 1
  fi
