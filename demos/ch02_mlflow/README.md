...
```bash
uv run --group lab python labs/ch02_model_quality/11_evaluate_and_record.py
```

MLflow까지 함께 확인하려면 다음처럼 실행합니다.

```bash
uv run --group demo python demos/ch02_mlflow/02_record_model_to_mlflow.py
```
...
```bash
MLFLOW_PORT=5001 bash demos/ch02_mlflow/01_run_with_docker_mlflow.sh
```
