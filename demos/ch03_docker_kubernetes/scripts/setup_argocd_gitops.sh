...
usage() {
  cat <<EOF
Usage: bash demos/ch03_docker_kubernetes/scripts/setup_argocd_gitops.sh <command>
...
prepare_key() {
  ...
  echo "bash demos/ch03_docker_kubernetes/scripts/setup_argocd_gitops.sh connect"
}
...
connect_repo_and_app() {
  ...
  if [[ ! -f "${KEY_PATH}" ]]; then
    ...
    echo "Run: bash demos/ch03_docker_kubernetes/scripts/setup_argocd_gitops.sh key" >&2
    exit 2
  fi
  ...
  echo "[next] Sync with: bash demos/ch03_docker_kubernetes/scripts/setup_argocd_gitops.sh sync"
}
