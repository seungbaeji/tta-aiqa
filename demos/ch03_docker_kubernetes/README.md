...
Argo CD가 바라볼 배포 파일은 `gitops/overlays/student` 아래에 있습니다. 이 overlay는 base의 MLflow tracking `Deployment`/`Service`/PVC와 KServe `InferenceService`를 함께 렌더링합니다.
...
`gitops/base`는 공통 배포 resource입니다. 수강생은 이 폴더를 직접 고치기보다 `gitops/overlays/student` 아래의 수업용 overlay를 수정합니다. 특히 MLflow UI를 노출할 ingress host는 각자 실습 VM, DNS, tunnel 주소가 다를 수 있으므로 다음 파일의 placeholder를 본인 주소로 바꿉니다.
...
| Application 등록 | Argo CD가 어떤 repo, branch, path를 cluster에 맞출지 선언 | `gitops/overlays/student`를 desired state로 삼기 위해 필요 |
...
| `gitops/base/mlflow-ingress.yaml` | MLflow UI용 ingress 기본 resource |
| `gitops/base/inferenceservice.yaml` | KServe candidate serving 선언 |
| `gitops/overlays/student/kustomization.yaml` | 수강생용 overlay |
| `gitops/overlays/student/ingress-host-patch.yaml` | 수강생이 본인 ingress 주소로 수정할 patch |
