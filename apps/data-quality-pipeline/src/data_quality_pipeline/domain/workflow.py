...
class SplitRevisionPaths:
    """Complete filesystem output locations for one approved split revision."""
...
class DataQualityPaths:
    ...
    def split_revision_paths(self) -> SplitRevisionPaths:
        """Return paths required for an approved split revision command."""
        values = (
            self.split_revision_config_path,
            self.revision_split_manifest_path,
            self.revision_split_dataset_dir,
        )
        if any(value is None for value in values):
            raise ValueError("split revision paths are required")
        return SplitRevisionPaths(
            config_path=self.split_revision_config_path,
            manifest_path=self.revision_split_manifest_path,
            dataset_dir=self.revision_split_dataset_dir,
        )
