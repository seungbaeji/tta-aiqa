"""Pydantic configuration adapter for the versioned Risk API YAML document."""
...
class RiskMetricNames(BaseModel):
    ...
    def validate_unique_names(self) -> "RiskMetricNames":
        """Reject configuration that would register duplicate metric names."""
...
class RiskMetricBuckets(BaseModel):
    ...
    def validate_sorted_buckets(self) -> "RiskMetricBuckets":
        """Require deterministic, non-empty bucket sequences for every histogram."""
...
class RiskApiObservabilityConfig(BaseModel):
    """Application-owned metric names, labels, buckets, and cardinality policy."""
    ...
    def validate_cardinality_policy(self) -> "RiskApiObservabilityConfig":
        """Reject label dimensions that could create unbounded metric cardinality."""
...
class ApiConfig(BaseModel):
    """Validated external API and observability configuration document."""
...
def load_api_config(path: Path) -> ApiConfig:
    """Load and validate one versioned Risk API YAML configuration document."""
