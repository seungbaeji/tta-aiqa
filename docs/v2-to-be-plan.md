...
외부 계약은 Risk API가 소유한다. Compose에서는 local sklearn adapter를 사용하고 k3s에서는 내부 KServe endpoint를 호출하는 adapter를 사용한다.
...
│   └── adapters/
│       ├── inbound/cli.py
│       └── outbound/grafana_cloud.py
├── tests/
├── pyproject.toml
└── README.md
```

Application layer는 dashboard UID/title, folder와 datasource binding을 계산한다. Grafana Cloud HTTP 요청과 인증은 outbound adapter만 담당하고 dashboard JSON은 structured data로 읽고 수정한다.
...
Domain package는 Grafana, Loki와 Tempo SDK 또는 dashboard API를 직접 사용하지 않는다. Runtime app은 log, metric과 trace를 표준 형식으로 노출하고 Alloy가 Grafana Cloud로 전달한다. Telemetry write credential은 Alloy 실행 환경에만 주입하고 dashboard API 호출은 Dashboard Importer app으로 격리한다.
...
| Grafana Dashboard Importer | App-local import use case와 Grafana Cloud HTTP adapter |
...
- Serving은 현재 input contract와 model bundle의 contract hash가 다르면 시작 또는 model 교체를 거부한다.
...
```text
reference/evidence/model-evaluation/
...
교육에서는 Compose의 Risk API와 Alloy를 먼저 실행해 local sklearn adapter, container log, service metrics와 OTLP 흐름을 확인한다. 이후 동일한 Risk API application과 telemetry label 계약을 Kubernetes로 옮기고 model adapter를 KServe HTTP로, telemetry discovery를 Kubernetes workload 기준으로 교체한다.
...
- MLflow에서 승인된 model bundle은 publish adapter가 VM의 course model PVC로 복사하고 KServe `storageUri`가 해당 immutable 경로를 가리킨다.
...
```text
dvc repro로 source 검증, aggregate, split과 traffic pool 생성
  -> GE raw/processed validation evidence 생성
  -> 내부 feature diagnostics와 canonical model input 확인
  -> baseline, Candidate A와 Candidate B bootstrap
  -> MLflow 기록
...
| telemetry label과 bucket 설정 | `configs/observability/telemetry.yaml` |
...
- `aiqa-observability`와 runtime app에서는 Grafana dashboard API와 backend별 배포 코드를 제거하고 Alloy가 받을 표준 telemetry까지만 구현한다.
...
Phase 0 실행 결과는 `reference/evidence/phase0/`에 보존한다. 현재 F0, F1과 F2는 모두 통과해 Phase 1 진행이 가능하다. 이 결론은 train/valid feasibility에 한정되며 Candidate B의 최종 `APPROVE`는 아니다. Phase 4에서 profile과 release policy를 freeze한 뒤 sealed test를 한 번 평가해야 canonical 교육 수치와 배포 여부가 확정된다.
...
- Compose와 Kubernetes의 Alloy가 각 환경의 Risk API telemetry를 동일한 label 계약으로 Grafana Cloud에 전달하며 local Grafana/Loki/Tempo/Prometheus server를 요구하지 않는다.
