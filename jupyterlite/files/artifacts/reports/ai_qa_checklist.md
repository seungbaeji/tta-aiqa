...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 데이터 품질 | pass | [x] | 라벨 허용값과 표본 수 확인 | label_basis_check.md | invalid=0, missing=0, high_risk=37, low_risk=33 | QA Lead | 배포 판단 보고서에 label basis 유지 근거로 첨부 |
...
| 서빙 품질 | pass | [x] | 준비된 API 계약 확인 | release_approval.md | prepared_api_contract=True, live 확인 결과와 분리 | Platform/MLOps | live smoke 확인 결과와 혼동하지 않도록 보고서에 한정 표현 |
| 서빙 품질 | unverified | [x] | live 배포 확인 결과 확인 | release_approval.md | unresolved_risks=live_deployment | Platform/MLOps | /health, /predict, Pod readiness, model_version, threshold 확인 결과 수집 |
