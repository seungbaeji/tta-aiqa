...
def revision_paths(revision: str) -> RevisionPaths:
    if revision == "v1":
        return RevisionPaths(
            profiles=ROOT / "configs/model-v1/profiles.yaml",
            evaluation=ROOT / "configs/model-v1/evaluation.yaml",
            feature_sets=ROOT / "configs/model-v1/feature-sets.yaml",
            release_policy=ROOT / "configs/qa/release-policy.yaml",
            split_dataset_dir=ROOT / "data/splits-v1",
            data_manifest=(
                ROOT / "docs/evidence/data-v2/data-manifest.json"
            ),
            evidence_root=ROOT / "docs/evidence/model-v1",
            artifact_root=ROOT / "artifacts/model",
            model_root=ROOT / "artifacts/models",
        )
    return RevisionPaths(
        profiles=ROOT / "configs/model-v2/profiles.yaml",
        evaluation=ROOT / "configs/model-v2/evaluation.yaml",
        feature_sets=ROOT / "configs/model-v2/feature-sets.yaml",
        release_policy=ROOT / "configs/qa-v2.yaml",
        split_dataset_dir=ROOT / "data/splits-v2",
        data_manifest=(
            ROOT / "docs/evidence/data-v2/split-revision.json"
        ),
        evidence_root=ROOT / "docs/evidence/model-v2",
