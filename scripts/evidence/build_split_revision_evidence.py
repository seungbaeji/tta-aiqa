...
def parse_args() -> argparse.Namespace:
    ...
    add_output_arguments(
        parser,
        root=ROOT,
        default_relative_path="data-v2/split-revision.json",
    )
...
def main() -> None:
    ...
    args = parse_args()
    parent_path = ROOT / "data/splits-v1/split-manifest.csv"
    revision_root = ROOT / "data/splits-v2"
    ...
    parent_evidence = json.loads(
        (ROOT / "docs/evidence/data-v2/data-manifest.json").read_text(
            encoding="utf-8"
        )
    )
    document = {
        ...
        "configuration": {
            ...
            "split_revision_sha256": sha256(
                ROOT / "configs/data/split-v2.yaml"
            ),
