...
def add_output_arguments(
    ...
    *,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="allow --output below reference/evidence after an explicit review",
    )
...
def resolve_output_path(
    *,
    ...
    historical_root = (root / "reference/evidence").resolve()
    if _is_within(resolved_output, historical_root) and not write_historical_evidence:
        raise ValueError(
            ...
            "reference/evidence or pass --write-historical-evidence explicitly"
        )
