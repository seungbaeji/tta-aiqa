...
| Great Expectations API 기초 Notebook | `labs/ch02_model_quality/1_great_expectations_api_basics_lab.ipynb` | 작은 예제로 GE API 기본 흐름 확인 |
| Great Expectations Notebook | `labs/ch02_model_quality/2_great_expectations_lab.ipynb` | validation 품질 검증 artifact 재생성 |
| 모델 평가 Notebook | `labs/ch02_model_quality/3_model_evaluation_lab.ipynb` | test 데이터 score, prediction, metric, threshold 해석 |
| 데이터-지표 연결 Notebook | `labs/ch02_model_quality/4_data_metric_connection_lab.ipynb` | 데이터 품질 신호와 metric 변화 연결 |
| MLflow Notebook | `labs/ch02_model_quality/5_mlflow_tracking_lab.ipynb` | 평가 기록과 재현 범위 확인 |
| 학습 script | `labs/ch02_model_quality/train_baseline.py` | 기준선 모델 artifact 생성 |
| 평가 기록 script | `labs/ch02_model_quality/evaluate_and_record.py` | test metric과 experiment 기록 생성 |
| 비교 artifact script | `labs/ch02_model_quality/build_comparison_artifacts.py` | validation degradation 비교 JSON/Markdown 생성 |
...
| Notebook | `labs/ch02_model_quality/3_model_evaluation_lab.ipynb` | 모델 학습, 기준선(baseline) 평가, 품질 저하 데이터셋 비교, 임계값 분석 실행 |
| CLI 학습 스크립트 | `labs/ch02_model_quality/train_baseline.py` | 기준선 모델 artifact 생성 |
...
준비된 artifact가 있는 환경에서는 먼저 `model_test_eval.json`과 `validation_degradation_comparison.json`을 확인합니다. `train_baseline.py`, `build_comparison_artifacts.py`, `uv run python scripts/course.py lab-model-quality`는 모델과 비교 산출물을 다시 만들 수 있으므로, 직접 실행하지 않았다면 보고서에는 “prepared artifact에서 확인한 값”이라고 적습니다.
...
이 실행은 같은 scikit-learn 모델의 test metric과 평가 조건을 하나의 기록으로 남깁니다. Notebook에서는 `labs/ch02_model_quality/3_model_evaluation_lab.ipynb`의 기준선 평가 셀을 실행합니다. 명령행에서는 먼저 2-4-1의 `train_baseline.py`로 기준선 모델을 만들고, 이어서 `evaluate_and_record.py`로 `artifacts/experiments/chapter_02/model_test_eval.json`에 기록합니다.
...
이 비교는 실행 여부와 관계없이 같은 artifact 값을 기준으로 QA 판단을 만들 수 있어야 합니다. Notebook에서는 `labs/ch02_model_quality/4_data_metric_connection_lab.ipynb`의 기준/품질 저하 데이터셋 비교 셀을 실행합니다. 읽기 전용으로 확인할 때는 `artifacts/experiments/chapter_02/validation_degradation_comparison.json`과 `artifacts/reports/chapter_02_model_quality_comparison.md`를 열어 같은 값을 확인합니다.
...
이 실행에서 확인할 핵심은 같은 점수에 임계값만 바꿔 적용했는지입니다. Notebook에서는 `labs/ch02_model_quality/3_model_evaluation_lab.ipynb`의 임계값 분석 셀을 실행합니다. 모델을 다시 학습하지 않고 같은 점수에 임계값만 바꿔 적용했는지 확인합니다.
...
| Notebook | `labs/ch02_model_quality/4_data_metric_connection_lab.ipynb` | 기준 데이터셋과 품질 저하 데이터셋 비교, 점수/예측 분포 확인 |
...
Notebook으로 실습한다면 `labs/ch02_model_quality/4_data_metric_connection_lab.ipynb`를 열고 첫 번째 셀부터 순서대로 실행합니다. 명령행에서는 `uv run python scripts/course.py lab-model-quality`로 2-4-1 기준선 모델과 2장 비교 산출물을 다시 만들 수 있습니다. 읽기 전용 리허설에서는 `validation_degradation_comparison.json`과 보고서용 Markdown을 열어 기준 validation 데이터셋과 품질 저하 validation 데이터셋의 지표, 점수 분위수, 예측 분포를 확인합니다.
