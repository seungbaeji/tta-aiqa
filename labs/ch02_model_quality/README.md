...
| 모델 평가 Notebook | `labs/ch02_model_quality/model_evaluation_lab.ipynb` | test 데이터 score, prediction, metric, threshold 해석 |
| 데이터-지표 연결 Notebook | `labs/ch02_model_quality/data_metric_connection_lab.ipynb` | 데이터 품질 신호와 metric 변화 연결 |
| Great Expectations Notebook | `labs/ch02_model_quality/great_expectations_lab.ipynb` | validation 품질 검증 artifact 재생성 |
| MLflow Notebook | `labs/ch02_model_quality/mlflow_tracking_lab.ipynb` | 평가 기록과 재현 범위 확인 |
...
| Notebook | `labs/ch02_model_quality/model_evaluation_lab.ipynb` | 모델 학습, 기준선(baseline) 평가, 품질 저하 데이터셋 비교, 임계값 분석 실행 |
...
이 실행은 같은 scikit-learn 모델의 test metric과 평가 조건을 하나의 기록으로 남깁니다. Notebook에서는 `labs/ch02_model_quality/model_evaluation_lab.ipynb`의 기준선 평가 셀을 실행합니다. 명령행에서는 먼저 2-4-1의 `train_baseline.py`로 기준선 모델을 만들고, 이어서 `evaluate_and_record.py`로 `artifacts/experiments/chapter_02/model_test_eval.json`에 기록합니다.
...
이 실행에서 확인할 핵심은 같은 점수에 임계값만 바꿔 적용했는지입니다. Notebook에서는 `labs/ch02_model_quality/model_evaluation_lab.ipynb`의 임계값 분석 셀을 실행합니다. 모델을 다시 학습하지 않고 같은 점수에 임계값만 바꿔 적용했는지 확인합니다.
...
| Notebook | `labs/ch02_model_quality/data_metric_connection_lab.ipynb` | 기준 데이터셋과 품질 저하 데이터셋 비교, 점수/예측 분포 확인 |
...
Notebook으로 실습한다면 `labs/ch02_model_quality/data_metric_connection_lab.ipynb`를 열고 첫 번째 셀부터 순서대로 실행합니다. 명령행에서는 `uv run python scripts/course.py lab-model-quality`로 2-4-1 기준선 모델과 2장 비교 산출물을 다시 만들 수 있습니다. 읽기 전용 리허설에서는 `validation_degradation_comparison.json`과 보고서용 Markdown을 열어 기준 validation 데이터셋과 품질 저하 validation 데이터셋의 지표, 점수 분위수, 예측 분포를 확인합니다.
