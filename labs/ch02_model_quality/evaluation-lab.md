...
준비된 artifact가 있는 환경에서는 먼저 `model_test_eval.json`과 `validation_degradation_comparison.json`을 확인합니다. `train_baseline.py`, `build_comparison_artifacts.py`, `make lab-model-quality`는 모델과 비교 산출물을 다시 만들 수 있으므로, 직접 실행하지 않았다면 보고서에는 “prepared artifact에서 확인한 값”이라고 적습니다.
...
```bash
uv run --group lab python labs/ch02_model_quality/train_baseline.py
```
