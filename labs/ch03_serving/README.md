...
3장은 예측 API가 score만 반환하는지 확인하는 데서 끝나지 않고, 요청/응답 계약, validation failure, train-serving skew가 운영 품질 증거로 남는지 확인하는 실습입니다.

## 실습 자료

| 구분 | 경로 | 역할 |
| --- | --- | --- |
| README | `labs/ch03_serving/README.md` | 3장 serving 실습 목적, 실행 순서, QA 해석 |
| Notebook | `labs/ch03_serving/fastapi_serving_lab.ipynb` | 요청 payload, 응답 schema, validation failure, train-serving skew 확인 |
| 계약 확인 script | `labs/ch03_serving/check_serving_contract.py` | FastAPI 계약 자동 확인과 prediction event 생성 |
