...
repository의 dashboard를 고정 UID로 생성합니다. Grafana, Loki, Tempo와 Prometheus
...
`.env.grafanacloud`에 입력합니다. 실제 값은 Git에 추가하지 않습니다.

## 3. 실행

### 3-1. Alloy 연결과 traffic
...
출력된 dashboard URL을 엽니다. 다시 실행해도 UID `tta-aiqa-quality`의 같은
dashboard가 갱신되어야 합니다.

### 3-3. Notebook

`01_inspect_dashboard_contract.ipynb`에서 telemetry metric과 dashboard query가
일치하는지 확인한 뒤, browser에서 baseline model version의 metric, log와 trace를
탐색합니다.
...
- logs에서 request ID, traces에서 같은 요청 흐름을 탐색할 수 있습니다.
- dashboard에 baseline `model_version` 데이터가 쌓입니다.

### 4-2. Local cleanup

Kubernetes release 실습으로 넘어가기 전에 local Compose workload를 정리합니다.
