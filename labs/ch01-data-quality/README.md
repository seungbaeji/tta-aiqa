...
이어집니다. 봉인된 최종 평가 데이터는 학습에 쓰지 않습니다.

### 1-1. train, valid, sealed test, operational의 역할을 누수 없이 구분한다

지금 데이터가 최신 선언과 같은지 본 뒤, 역할 표를 읽습니다. 현재 역할은
`train 2,900 / valid 600 / test 400 / operational 100`입니다. 실행 전에 개발,
봉인 평가, 정답 없는 운영 표본의 역할을 예측한 뒤 표와 대조합니다. 이전 분할
...
`labs/exercises/solutions/ge_summary/interpret.py`를 참고할 수 있지만, 정답
...
선언 파일과 대조합니다. 봉인된 `test`와 `operational` 파일은 열지 않습니다.
잠금 파일 지문이 달라도 값을 고치지 말고 계산 결과를 적습니다.

## 3. 단계 완료

판단 기록의 데이터 품질 칸에 역할 구분, 대표 품질 관찰, 검증 명령과 결과
범위를 남깁니다. 이 기록은 다음 장에서 같은 데이터 범위와 봉인 경계를
이어받는지 확인하는 근거입니다.
