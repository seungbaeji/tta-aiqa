...
def test_sealed_roles_cannot_be_read_without_explicit_access(tmp_path: Path) -> None:
    adapter = benchmark(tmp_path)

    with pytest.raises(PermissionError, match="sealed dataset role"):
        adapter._read_role("test")
    with pytest.raises(PermissionError, match="sealed dataset role"):
        adapter._read_role("operational")
...
def test_final_confirmation_scores_supplied_serialized_pipeline(
    ...
    result = benchmark(tmp_path).final_confirmation(
        ...
        fitted_pipelines={"baseline": FittedPipeline()},
    )
