...
모델 예측 event 이름, metric 이름·label·bucket, traffic scenario, dashboard query는 각 app의 책임입니다. 예를 들어 Risk API의 metric 정책은 `configs/serving/api.yaml`에 있고, dashboard import는 `apps/grafana_dashboard_importer`가 담당합니다. Grafana, Loki, Tempo와 Prometheus server의 배포도 이 package의 책임이 아닙니다.
