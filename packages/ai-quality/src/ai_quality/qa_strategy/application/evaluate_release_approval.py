...
def evaluate_release_approval(
    ...
    check_results: tuple[ApprovalCheckResult, ...] = (
        ...
        ApprovalCheckResult(
            ...
            criterion="local/prepared contract evidence is True",
        ...
        unresolved_risks.append(
            ApprovalRisk(
                ...
                evidence=(
                    ...
                    "or threshold evidence in the local course artifact."
                ),
                owner="Platform/MLOps",
                next_action=(
                    "Collect live deployment smoke-test evidence before using "
    ...
    risk_tradeoffs = (
        ReleaseRiskTradeoff(
            ...
            owner="QA Lead",
            next_action=(
                "승인하지 않고 owner별 확인 결과를 release gate에 "
                "재상정합니다."
            ),
        ),
        ReleaseRiskTradeoff(
            ...
            missing_evidence=(
                "owner별 next action 완료 결과와 같은 approval rule 재평가"
            ),
            owner="Release Owner",
            next_action=(
    ...
    return ApprovalDecision(
        ...
        re_evaluation_condition=(
            "failed_checks와 unresolved_risks가 해소되고 owner별 evidence가 "
            "같은 approval rule을 통과하면 재평가합니다."
        ),
    )
